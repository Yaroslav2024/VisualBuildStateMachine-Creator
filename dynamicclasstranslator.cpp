...
DynamicClassTranslator::DynamicClassTranslator(QObject *parent) : QObject(parent) {}

ClassResult DynamicClassTranslator::generate(QGraphicsScene *scene, const QString &className, const QString &manualHeader)
{
    ClassResult result;
    QMap<QString, VariableInfo> classVars; // Переменные, которые станут полями класса

    // 1. СБОР БЛОКОВ (Ищем Start и остальные)
    struct BlockData {
        DiagramItem* item;
        int id;
    ...
    for (int i = 0; i < blocks.size(); ++i) {
        ...
        // ВАЖНО: Сохраняем связь ID -> Блок, чтобы дебаггер мог красить блоки в зеленый
        result.idMap.insert(blocks[i].id, blocks[i].item);

        // Чистим код внутри блоков и вытаскиваем переменные в поля класса
    ...
    // ==========================================
        ...
        cppOut << "    // Wait for the cable to be connected/the port to be opened indefinitely (Start on trigger)\n";
        cppOut << "    while(!Serial) { delay(10); }\n";
        cppOut << "    delay(500);\n";
        cppOut << "    Serial.println(\"--- DEBUG SESSION [\" << className << \"] ---\");\n";
        cppOut << "    FSM_Init();\n";
        cppOut << "}\n\n";

        // --- Протокол Дебаггера ---
        // --- Протокол Дебаггера (Асинхронный / Неблокирующий) ---
        cppOut << "void " << className << "::dbg_sig(char type, int id) {\n";
        cppOut << "    if (Serial.availableForWrite() >= 16) {\n";
        cppOut << "        Serial.print(\"[\"); Serial.print(type); Serial.print(\":\");\n";
        cppOut << "        Serial.print(id); Serial.println(\"]\");\n";
        cppOut << "    }\n";
        cppOut << "}\n\n";
        ...
        cppOut << "            (this->*(activeFSMs[i].current))(i); // OnExit\n";
        cppOut << "            }\n";
        ...
        cppOut << "        } else {\n";
        cppOut << "            (this->*(activeFSMs[i].current))(i); // OnEnter & OnLoop\n";
        cppOut << "        }\n";
        cppOut << "    }\n";
        cppOut << "}\n\n";
        ...
        for (const auto& block : blocks) {
            ...
            if (BridgeItem* bridge = dynamic_cast<BridgeItem*>(block.item)) {
                ...
                if (targetId != -1) {
                    ...
                    cppOut << "    Serial.println(\"Error: Bridge target not found!\");\n";
            ...
            if (!hasUncond && transitions.isEmpty()) {
                 cppOut << "    Serial.println(\"Finished.\");\n";
                 cppOut << "    requestTransition(i, nullptr);\n";
            }
            cppOut << "}\n\n";
        }
        ...
        mainOut << "// --- VisioLite Dynamic Debugger (Class Mode) ---\n";
...
// === Утилиты ===
QString DynamicClassTranslator::sanitizeForArduino(const QString &rawCode, bool isCondition) {
...
QString DynamicClassTranslator::extractVariables(const QString &rawCode, QMap<QString, VariableInfo> &vars) {
    QString code = rawCode;
    QRegularExpression varRegex(R"(\b(int|float|double|bool|String|char|long)\s+(\w+)(\s*=[^;]+)?\s*;)");
