...
{
    // ======================================================
    // --- ИСПРАВЛЕНО: Кроссплатформенные имена и пути ---
    // ======================================================
#ifdef Q_OS_WIN
    QString exeName = toolName + ".exe";
    QString exeFilter = QObject::tr("Executables (*.exe);;All Files (*)");
    QString defaultBrowsePath = "C:/Program Files";
#else
    QString exeName = toolName; // На Linux/Mac нет расширения .exe
    QString exeFilter = QObject::tr("Executables (*);;All Files (*)");
    QString defaultBrowsePath = "/usr/lib/jvm";
#endif
...
#ifndef Q_OS_WIN
        // Дополнительные пути только для Linux/Mac:
        searchRoots << "/usr/lib/jvm"
...
#ifndef Q_OS_WIN
                    // Специфичный путь для macOS внутри пакета
                    if (!QFile::exists(candidate)) {
                         candidate = root + "/" + latest + "/Contents/Home/bin/" + exeName;
                    }
#endif
...
bool Utils::compileJava(const QString &sourceCode, const QString &tempDir, QWidget *parent)
{
    ...
    // =================================================================
    // --- ИДЕАЛЬНЫЙ ПОИСК БИБЛИОТЕК ДЛЯ JAVA (CLASSPATH) ---
    // =================================================================
    QString ideLibPath = QCoreApplication::applicationDirPath() + "/JavaLibraries";
    QString currentProjectDir = settings.value("lastProject", "").toString(); // ТОЧНЫЙ ПУТЬ ИЗ MAINWINDOW
    QString customLibPath = settings.value("LibraryManager/LastSavedPath", "").toString();

    // Выбираем правильный разделитель путей для Java (Windows = ";", Linux/Mac = ":")
    #ifdef Q_OS_WIN
        QString pathSep = ";";
    #else
        QString pathSep = ":";
    ...
    // Формируем Classpath (пути, где Java будет искать .jar и .class файлы)
    QString classPath = "." + pathSep + ideLibPath + "/*";

    // Добавляем папку открытого проекта
    if (!currentProjectDir.isEmpty()) {
        classPath += pathSep + currentProjectDir + pathSep + currentProjectDir + "/JavaLibraries/*";
    }

    // Добавляем папку из онлайн-менеджера
    if (!customLibPath.isEmpty()) {
        classPath += pathSep + customLibPath + "/*";
    }
    // =================================================================
...
bool Utils::compileCpp(const QString &sourceCode, const QString &exePath, QWidget *parent)
{
    ...
    // =================================================================
    // --- ИДЕАЛЬНЫЙ ПОИСК БИБЛИОТЕК ---
    // =================================================================
    QSettings settings("VisualBuild", "IDE");

    // 1. Системная папка IDE
    QString ideLibPath = QCoreApplication::applicationDirPath() + "/CppLibraries";

    // 2. БЕРЕМ ТОЧНЫЙ ПУТЬ ТЕКУЩЕГО ПРОЕКТА ИЗ MAINWINDOW!
    QString currentProjectDir = settings.value("lastProject", "").toString();

    // 3. Папка из онлайн-загрузчика
    QString customLibPath = settings.value("LibraryManager/LastSavedPath", "").toString();
    ...
    // Подключаем системную папку
    args << "-I" << ideLibPath;

    // Подключаем папку проекта (например, test_1) и папку CppLibraries внутри нее
    if (!currentProjectDir.isEmpty()) {
        args << "-I" << currentProjectDir;
        args << "-I" << (currentProjectDir + "/CppLibraries");
    }

    // Подключаем папку загрузок (Документы)
    if (!customLibPath.isEmpty()) {
        args << "-I" << customLibPath;
    }
    // =================================================================
    ...
    if (QFileInfo(compilerCmd).isAbsolute()) {
        isCompilerValid = QFile::exists(compilerCmd);
    } else {
        isCompilerValid = !QStandardPaths::findExecutable(compilerCmd).isEmpty();
    }
    ...
    if (compiler.exitCode() != 0) {
        ...
        g_lastCompileError = QString::fromLocal8Bit(errData);
...
bool Utils::checkPythonSyntax(const QString &filePath, QWidget *parent)
{
    ...
    // =================================================================
    // --- ИДЕАЛЬНЫЙ ПОИСК БИБЛИОТЕК ДЛЯ PYTHON (PYTHONPATH) ---
    // =================================================================
    QString ideLibPath = QCoreApplication::applicationDirPath() + "/PythonLibraries";
    QString currentProjectDir = settings.value("lastProject", "").toString();
    QString customLibPath = settings.value("LibraryManager/LastSavedPath", "").toString();
    ...
    // Добавляем папку проекта и папку библиотек внутри него
    if (!currentProjectDir.isEmpty()) {
        pyPaths << currentProjectDir;
        pyPaths << (currentProjectDir + "/PythonLibraries");
    }

    // Добавляем папку из онлайн загрузчика
    if (!customLibPath.isEmpty()) {
        pyPaths << customLibPath;
    }

    // Выбираем правильный разделитель путей для ОС (Windows = ";", Linux/Mac = ":")
    #ifdef Q_OS_WIN
        QString pathSep = ";";
    #else
        QString pathSep = ":";
    ...
    QString customPythonPath = pyPaths.join(pathSep);

    // Формируем безопасное окружение
    QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
    env.remove("PYTHONHOME"); // Защита от конфликтов версий Python

    // Умное слияние с уже существующим системным PYTHONPATH (если он есть)
    QString existingPyPath = env.value("PYTHONPATH");
    if (!existingPyPath.isEmpty()) {
        customPythonPath = existingPyPath + pathSep + customPythonPath;
    }
    env.insert("PYTHONPATH", customPythonPath);
    // =================================================================
    ...
    // ========================================================
    QProcess process;
    process.setProcessEnvironment(env); // <-- ПРИМЕНЯЕМ НАШИ ПУТИ К ПРОЦЕССУ

    // Запускаем проверенный python
    process.start(pyCmd, QStringList() << "-m" << "py_compile" << filePath);

    // ========================================================
    // --- Защита от зависания базовой проверки ---
    // ========================================================
    if (!process.waitForFinished(5000)) { // Ждем максимум 5 секунд
        QMessageBox::critical(parent, QObject::tr("Ошибка Python"),
                              QObject::tr("Процесс Python не отвечает или завис.\nПроверка синтаксиса прервана."));
        return false;
    }
    ...
    QProcess scanner;
    scanner.setProcessEnvironment(env); // <-- ПРИМЕНЯЕМ НАШИ ПУТИ И К СКАНЕРУ ТОЖЕ

    // Запускаем пользовательский проверенный python
    scanner.start(pyCmd, QStringList() << scannerScriptPath << filePath);

    // ========================================================
    // --- Защита от зависания AST-сканера ---
    // ========================================================
    if (!scanner.waitForFinished(5000)) { // Ждем максимум 5 секунд
        QMessageBox::critical(parent, QObject::tr("Ошибка защиты"),
                              QObject::tr("AST-сканер Python не отвечает или завис.\nПроверка прервана."));
        return false;
    }
...
bool Utils::checkCppSyntax(const QString &sourceCode, const QString &headerCode, const QString &headerName, QWidget *parent)
{
    ...
    // =================================================================
    // --- ИДЕАЛЬНЫЙ ПОИСК ДЛЯ ПРОВЕРКИ СИНТАКСИСА ---
    // =================================================================
    QString ideLibPath = QCoreApplication::applicationDirPath() + "/CppLibraries";
    QString currentProjectDir = settings.value("lastProject", "").toString();
    QString customLibPath = settings.value("LibraryManager/LastSavedPath", "").toString();
    ...
    QStringList args;
    args << "-std=c++17" << "-fsyntax-only" << "temp_check.cpp";
    ...
    if (!compiler.waitForFinished(3000)) {
        return false;
    }
...
bool Utils::checkJavaSyntax(const QString &sourceCode, const QString &className, QWidget *parent)
{
    ...
    // =================================================================
    // --- ИДЕАЛЬНЫЙ ПОИСК ДЛЯ ПРОВЕРКИ СИНТАКСИСА JAVA ---
    // =================================================================
    QSettings settings("VisualBuild", "IDE");
    QString ideLibPath = QCoreApplication::applicationDirPath() + "/JavaLibraries";
    // БЕРЕМ ТОЧНЫЙ ПУТЬ ИЗ ПАМЯТИ IDE, а не из QDir::currentPath()!
    QString currentProjectDir = settings.value("lastProject", "").toString();
    QString customLibPath = settings.value("LibraryManager/LastSavedPath", "").toString();

    // Автоматический выбор разделителя в зависимости от ОС
    #ifdef Q_OS_WIN
        QString pathSep = ";";
    #else
        QString pathSep = ":";
    ...
    // Подключаем папку проекта и библиотеки внутри него
    if (!currentProjectDir.isEmpty()) {
        classPath += pathSep + currentProjectDir + pathSep + currentProjectDir + "/JavaLibraries/*";
    }

    // Подключаем папку онлайн-библиотек (из Документов)
    if (!customLibPath.isEmpty()) {
        classPath += pathSep + customLibPath + "/*";
    }
    // =================================================================
    ...
    // ========================================================
    // --- Защита от зависания проверки (3 секунды) ---
    // ========================================================
    if (!compiler.waitForFinished(3000)) {
        return false; // Тихо отменяем проверку, чтобы не повесить интерфейс
    }
