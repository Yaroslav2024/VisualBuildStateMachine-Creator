...
void DebuggerWindow::onProcessOutputReady()
{
    ...
    // --- ИЗМЕНЕНИЕ: Универсальный заголовок ошибки ---
...
{
    // 1. Снимаем подсветку со старого (используем m_currentBlockItem)
    if (m_currentBlockItem) {
        m_currentBlockItem->setBrush(Qt::white); // Возвращаем белый цвет
        m_currentBlockItem = nullptr;
    }

    // 2. Ищем новый по ID (используем m_itemMap вместо m_idMap)
    if (m_itemMap.contains(id)) {
        QGraphicsItem *item = m_itemMap[id];
        if (DiagramItem *block = dynamic_cast<DiagramItem*>(item)) {
            m_currentBlockItem = block; // Запоминаем новый
            block->setBrush(Qt::green); // Подсветка активного
...
{
    // Используем m_itemMap вместо m_idMap
    if (m_itemMap.contains(id)) {
        QGraphicsItem *item = m_itemMap[id];
        if (ConnectionItem *arrow = dynamic_cast<ConnectionItem*>(item)) {
            if (success) {
                arrow->setHighlight(true); // Зеленая
                // Используем QTimer (теперь библиотека подключена)
                QTimer::singleShot(500, [arrow](){ arrow->setHighlight(false); });
            } else {
                arrow->setFalsePath(true); // Красная
                QTimer::singleShot(500, [arrow](){ arrow->setFalsePath(false); });
            }
        }
    }
}
