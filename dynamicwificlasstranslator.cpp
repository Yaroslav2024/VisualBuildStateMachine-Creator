...
ClassResult DynamicWifiClassTranslator::generate(QGraphicsScene *scene, const QString &className, const QString &manualHeader, const QString &ssid, const QString &pass)
{
    ...
    // ==========================================
        ...
        // --- Setup ---
                ...
                cppOut << "    \n";
                cppOut << "    // We are waiting endlessly to connect to the router\n";
                cppOut << "    while (WiFi.status() != WL_CONNECTED) delay(500);\n";

                // НОВОЕ: ВЫВОДИМ IP АДРЕС В ПОРТ!
                cppOut << "    Serial.println(\"\\n[Wi-Fi] Connected!\");\n";
                cppOut << "    Serial.print(\"[Wi-Fi] IP Address: \");\n";
                cppOut << "    Serial.println(WiFi.localIP());\n";

                cppOut << "    server->begin();\n";
                cppOut << "    \n";

                // НОВОЕ: АВТОНОМНЫЙ СТАРТ БЕЗ БЛОКИРОВКИ!
                cppOut << "    Serial.println(\"Autonomous start in 2 seconds...\");\n";
                cppOut << "    delay(2000);\n";
                cppOut << "    FSM_Init();\n";
                cppOut << "}\n\n";

        // --- Протокол Дебаггера (По воздуху - Асинхронный) ---
        ...
        // --- CORE FSM LOGIC ---
                ...
                cppOut << "    // Восстанавливаем связь, если IDE переподключилась на лету\n";
                cppOut << "    if (!client || !client.connected()) {\n";
                cppOut << "        WiFiClient newClient = server->available();\n";
                cppOut << "        if (newClient) client = newClient;\n";
                cppOut << "    }\n";
